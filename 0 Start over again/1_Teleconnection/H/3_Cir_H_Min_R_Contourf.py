...
# --- Read data --- #
f1 = xr.open_dataset(r'F:\6_Scientific_Research\3_Teleconnection\0 Start over again\0_Data\1_Teleconnection\H\Min_R.nc')
f2 = xr.open_dataset(r'F:\6_Scientific_Research\3_Teleconnection\0 Start over again\0_Data\1_Teleconnection\H\lat_min.nc')
f3 = xr.open_dataset(r'F:\6_Scientific_Research\3_Teleconnection\0 Start over again\0_Data\1_Teleconnection\H\lon_min.nc')
...
# --- Tel—point --- #
HTel = pd.DataFrame(columns=['Min_r', 'A_lat', 'A_lon', 'B_lat', 'B_lon'])
A_lat = [87.5, 87.5, 50, 45, 27.5, 60, 67.5,  40, 35, 52.5]
A_lon = [87.5, 105, 260, 50, 32.5, 127.5, 320, 30, 70, 17.5]
Min_r, B_lat, B_lon = [], [], []
for i in range(10):
    longitude = A_lon[i]
    latitude = A_lat[i]
...
HTel.Min_r, HTel.A_lat, HTel.A_lon, HTel.B_lat, HTel.B_lon = Min_r, A_lat, A_lon, B_lat, B_lon
HTel.to_csv(r'F:\6_Scientific_Research\3_Teleconnection\0 Start over again\0_Data\1_Teleconnection\H\HTel.csv')
...
# for i in range(27):
#     for j in range(144):
#         if r[i, j] >= -0.670 and r[i, j] <= -0.665:
...
ax_cf1 = ax1.contourf(cyclic_lons, lat, r, transform=ccrs.PlateCarree(), cmap='hot', levels=np.arange(-0.75, -0.35, 0.05 ), extend='both')  # 绘制等值线图
