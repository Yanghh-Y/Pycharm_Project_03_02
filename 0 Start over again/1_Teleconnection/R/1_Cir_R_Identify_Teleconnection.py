    ...
    for ilon in range(0, 144):# ilon [0, 360, 2.5]
        for dlat in range(0, 27):
            for dlon in range(0, 144):
                Cor_R[ilat, ilon, dlat, dlon], Cor_P[ilat, ilon, dlat, dlon] = pearsonr(Cir_R_Sum[:, ilat, ilon], Cir_R_Sum[:, dlat, dlon])
...
dr.to_netcdf(r'F:\6_Scientific_Research\3_Teleconnection\0 Start over again\0_Data\1_Teleconnection\R\Cor_R.nc')
Cor_P = xr.DataArray(Cor_P, coords=[ilat, ilon, dlat, dlon], dims=['ilat', 'ilon', 'dlat', 'dlon'])
dp = xr.Dataset({'Cor_R': Cor_R})
dp.to_netcdf(r'F:\6_Scientific_Research\3_Teleconnection\0 Start over again\0_Data\1_Teleconnection\R\Cor_P.nc')
...
ds.to_netcdf(r'F:\6_Scientific_Research\3_Teleconnection\0 Start over again\0_Data\1_Teleconnection\R\Min_R.nc')
Min_P = xr.DataArray(Min_P, coords=[lat, lon], dims=['lat', 'lon'])
da = xr.Dataset({'Min_p': Min_P})
da.to_netcdf(r'F:\6_Scientific_Research\3_Teleconnection\0 Start over again\0_Data\1_Teleconnection\R\Min_p.nc')
lat_min = xr.DataArray(lat_min, coords=[lat, lon], dims=['lat', 'lon'])
dla = xr.Dataset({'lat_min': lat_min})
dla.to_netcdf(r'F:\6_Scientific_Research\3_Teleconnection\0 Start over again\0_Data\1_Teleconnection\R\lat_min.nc')
lon_min = xr.DataArray(lon_min, coords=[lat, lon], dims=['lat', 'lon'])
dlo = xr.Dataset({'lon_min': lon_min})
dlo.to_netcdf(r'F:\6_Scientific_Research\3_Teleconnection\0 Start over again\0_Data\1_Teleconnection\R\lon_min.nc')
