...
Z_Recon_21,Z_Recon_22 = np.zeros((91, 360)), np.zeros((91, 360))
# --- 提取线性分量 --- #
...
for ilat in range(91):
    for ilon in range(360):
        ...
        Z_Recon_21[ilat, ilon] = linreg.predict(X_0[42,:].reshape(1, -1))
        Z_Recon_22[ilat, ilon] = linreg.predict(X_0[43,:].reshape(1, -1))


# --- Drawing - Contourf --- #
# -- Data - Pre-Process -- #
Z_Recon_21 = Z_Recon_21 + Z_linear_2021
Z_Recon_22 = Z_Recon_22 + Z_linear_2022
Z_Recon_21_mask = mask_land(Z_Recon_21, label='ocean', lonname='longitude')
Z_Recon_22_mask = mask_land(Z_Recon_22, label='ocean', lonname='longitude')
Z_Recon_21c, cyclic_lons = add_cyclic_point(Z_Recon_21_mask, coord=lon)
Z_Recon_22c, cyclic_lons = add_cyclic_point(Z_Recon_22_mask, coord=lon)
# -- Fuction - Drawing -- #
def drawing_Z(cyclic_data1, cyclic_data2, name1, name2):
    ...
    plt.savefig(r'F:\6_Scientific_Research\3_Teleconnection\0 Start over again\1_Picture\2_Reg_Recon-Pred\Muliti-Reg-Reconstruct-Z-2021-2022.png')
    plt.show()
drawing_Z(Z_Recon_21c, Z_Recon_22c, name1='Reconstruct-21', name2='Reconstruct-22')
