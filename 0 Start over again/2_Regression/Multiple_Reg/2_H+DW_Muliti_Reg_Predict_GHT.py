...
# --- Read - Data --- #
# ----------Z
f1 = xr.open_dataset(r'F:\6_Scientific_Research\3_Teleconnection\1_Data\ERA5\ERA5_GHT_1979-2022_JJA_1_1.nc')
Z = f1.z.sel(expver=1, level=500).groupby('time.year').mean(dim='time', skipna=True) / 9.8
lat = Z.latitude
lon = Z.longitude
Z_CL = Z.loc[1979:2000].mean('year') # 气候态平均
# --- Calculating - Z_ANO --- #
Z_ANO = np.zeros((44, 91, 360))
Z_detrend = scipy.signal.detrend(Z, axis=0, type='linear', overwrite_data=False)
Z_year_mean = Z.mean('year')
for iyear in range(44):
    Z_detrend[iyear, :, :] = Z_detrend[iyear, :, :] + Z_year_mean[:, :]
    Z_ANO[iyear, :, :] = Z_detrend[iyear, :, :] - Z_CL[:, :]
Z_Predict_21,Z_Predict_22 = np.zeros((91, 360)), np.zeros((91, 360))
# --- 提取线性分量 --- #
Z_linear = Z - Z_detrend
Z_linear_2021 = Z_linear[-2, :, :]
Z_linear_2022 = Z_linear[-1, :, :]
...
for ilat in range(91):
    for ilon in range(360):
        ...
        Y_0 = np.array(Z_ANO[:, ilat, ilon]) # 某一格点上的温度异常序列
        # --- Predict - 2021
        X2021 = X_0[0:42, :]
        Y2021 = Y_0[0:42]
        ...
        Z_Predict_21[ilat, ilon] = linreg2021.predict(X_0[42,:].reshape(1, -1))
        ...
        Z_Predict_22[ilat, ilon] = linreg2022.predict(X_0[43,:].reshape(1, -1))


# --- Drawing - Contourf --- #
# -- Data - Pre-Process -- #
Z_Predict_21 = Z_Predict_21 + Z_linear_2021
Z_Predict_22 = Z_Predict_22 + Z_linear_2022
Z_Predict_21_mask = mask_land(Z_Predict_21, label='ocean', lonname='longitude')
Z_Predict_22_mask = mask_land(Z_Predict_22, label='ocean', lonname='longitude')
Z_Predict_21c, cyclic_lons = add_cyclic_point(Z_Predict_21_mask, coord=lon)
Z_Predict_22c, cyclic_lons = add_cyclic_point(Z_Predict_22_mask, coord=lon)
# -- Fuction - Drawing -- #
def drawing_Z(cyclic_data1, cyclic_data2, name1, name2):
    ...
    # 图名
    ax1.set_title('H+W_Muliti_Reg-'+str(name1)+'-Z')
    ax2.set_title('H+W_Muliti_Reg-'+str(name2)+'-Z')
    # 刻度线形式
    ...
    ax_cf1 = ax1.contourf(cyclic_lons, lat, cyclic_data1, transform=ccrs.PlateCarree(), cmap='coolwarm', levels=np.arange(-40, 45, 5), extend='both')  # 绘制等值线图
    ax_cf2 = ax2.contourf(cyclic_lons, lat, cyclic_data2, transform=ccrs.PlateCarree(), cmap='coolwarm', levels=np.arange(-40, 45, 5), extend='both')  # 绘制等值线图
    fig.subplots_adjust(right=0.9)
    position = fig.add_axes([0.93, 0.25, 0.015, 0.5])  # 位置[左,下,右,上]
    cb = fig.colorbar(ax_cf2, shrink=0.6, cax=position, extend='both')
    plt.savefig(r'F:\6_Scientific_Research\3_Teleconnection\0 Start over again\1_Picture\2_Reg_Recon-Pred\Muliti-Reg-Predict-Z-2021-2022.png')
    plt.show()
drawing_Z(Z_Predict_21c, Z_Predict_22c, name1='Predict-21', name2='Predict-22')
