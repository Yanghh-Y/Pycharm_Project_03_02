...
SAT_Predict_21,SAT_Predict_22 = np.zeros((91, 360)), np.zeros((91, 360))
# --- 提取线性分量 --- #
...
TELIS_R = pd.read_csv(r'F:\6_Scientific_Research\3_Teleconnection\0 Start over again\0_Data\1_Teleconnection\R\Tel_R_I.csv',
                      usecols=['A1A2', 'A3A4', 'B1B2', 'C1C2', 'D1D2', 'E1E2', 'F1F2'])

TELI = pd.concat([TELIS_H, TELIS_W, TELIS_R], axis=1)
col = ['H_A1A2', 'H_A3A4', 'H_B1B2', 'H_C1C2', 'H_C3C4', 'H_D1D2', 'H_E1E2', 'H_F1F2', 'H_G1G2', 'H_H1H2', 'W_A1A2', 'W_A3A4', 'W_B1B2', 'W_B3B4', 'W_C1C2', 'W_D1D2', 'W_E1E2',
       'W_F1F2', 'R_A1A2', 'R_A3A4', 'R_B1B2', 'R_C1C2', 'R_D1D2', 'R_E1E2', 'R_F1F2']
TELI.columns = col
...
for ilat in range(91):
    for ilon in range(360):
        ...
        SAT_Predict_21[ilat, ilon] = linreg2021.predict(X_0[42,:].reshape(1, -1))
        ...
        SAT_Predict_22[ilat, ilon] = linreg2022.predict(X_0[43,:].reshape(1, -1))
...
def drawing_SAT(cyclic_data1, cyclic_data2, name1, name2):
    ...
    # 图名
    ax1.set_title('H+W+R_Muliti_Reg-'+str(name1)+'-SAT')
    ax2.set_title('H+W+R_Muliti_Reg-'+str(name2)+'-SAT')
    # 刻度线形式
    ...
    plt.savefig(r'F:\6_Scientific_Research\3_Teleconnection\0 Start over again\1_Picture\2_Reg_Recon-Pred\H+W+R+Muliti-Reg-Predict-SAT-2021-2022.png')
    plt.show()
