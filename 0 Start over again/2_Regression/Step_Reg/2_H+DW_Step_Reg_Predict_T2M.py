...
T2M_Pred_21,T2M_Pred_22 = np.zeros((91, 360)), np.zeros((91, 360))
# --- 提取线性分量 --- #
...
for ilat in range(91):
    for ilon in range(360):
        ...
        try:
            ...
            T2M_Pred_22[ilat, ilon] = predict2022
            # --- Pred 2021
            ...
            T2M_Pred_21[ilat, ilon] = predict2021

        except:
            tuple = (ilat, ilon)
            ErrList.append(tuple)
            T2M_Pred_21[ilat, ilon], T2M_Pred_22[ilat, ilon] = T2M_ANO[42, ilat, ilon], T2M_ANO[43, ilat, ilon]


# --- Drawing - Contourf --- #
# -- Data - Pre-Process -- #
T2M_Pred_21 = T2M_Pred_21 + T2M_linear_2021
T2M_Pred_22 = T2M_Pred_22 + T2M_linear_2022
T2M_Pred_21_Mask = mask_land(T2M_Pred_21, label='ocean', lonname='longitude')
T2M_Pred_22_Mask = mask_land(T2M_Pred_22, label='ocean', lonname='longitude')
T2M_Pred_21c, cyclic_lons = add_cyclic_point(T2M_Pred_21_Mask, coord=lon)
T2M_Pred_22c, cyclic_lons = add_cyclic_point(T2M_Pred_22_Mask, coord=lon)
# -- Fuction - Drawing -- #
def drawing_T2M(cyclic_data1, cyclic_data2, name1, name2):
    ...
    plt.savefig(r'F:\6_Scientific_Research\3_Teleconnection\0 Start over again\1_Picture\2_Reg_Recon-Pred\Predict-T2M-2021-2022.png')
    plt.show()
drawing_T2M(T2M_Pred_21c, T2M_Pred_22c, name1='Predict-21', name2='Predict-22')
