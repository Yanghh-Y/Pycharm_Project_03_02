...
for k in range(7): # 这一纬度是存放不同的遥相关指数对
    for i in range(37):
        for j in range(144):
            s[k, i, j], _, r[k, i, j], p[k ,i, j], _ = linregress(TELIS.iloc[:, k], hgt_detrend[:, i, j])
...
def drawingcount(Data, P,abc):
    ...
    fig.savefig(r'F:\6_Scientific_Research\3_Teleconnection\0 Start over again\1_Picture\3_Explanation\2_Reg\reg_H-tel_GHT.png')
    plt.show()
