...
u = f2.data[Sum_Index, :, 0:37, :]
w = f3.data[Sum_Index, :, 0:37, :] * (-300)
Cir_W_Sum, us, ws = np.zeros((44, 17, 37, 144)), np.zeros((44, 17, 37, 144)), np.zeros((44, 17, 37, 144))
n = 0
for i in range(132): # 计算夏季平均
    if i%3 == 0:
        Cir_W_Sum[n, :, :, :] = (Cir_W[i, :, :, :] + Cir_W[i + 1, :, :, :] + Cir_W[i + 2, :, :, :]) / 3
        us[n, :, :, :] = (u[i, :, :, :] + u[i + 1, :, :, :] + u[i + 2, :, :, :]) / 3
        ws[n, :, :, :] = (w[i, :, :, :] + w[i + 1, :, :, :] + w[i + 2, :, :, :]) / 3
        n = n + 1
# Cir_W_Sum[44, 17, 37, 144], [year, height, lat, lon]
# 后续的操作需要Cir_W_Sum是一个DataArray
height, lat, lon = Cir_W.height, Cir_W.lat, Cir_W.lon
year = range(1979, 2023)

# --------计算回归 --------- #
Hr, P, ur, wr = np.zeros((7, 17, 37, 144)), np.zeros((7, 17, 37, 144)), np.zeros((7, 17, 37, 144)), np.zeros((7, 17, 37, 144))
for k in range(7):
    print(k)
    TelI = TELIS.iloc[:, k]
    for h in range(17):
        for i in range(37):
            for j in range(144):
                Hr[k, h, i, j], _, _, P[k, h, i, j], _ = linregress(TelI, Cir_W_Sum[:, h, i, j])
                ur[k, h, i, j], _, _, _, _ = linregress(TelI, us[:, h, i, j])
...
ur = xr.DataArray(ur, coords=[itel, height, lat, lon], dims=['itel', 'height', 'lat', 'lon'])
wr = xr.DataArray(wr, coords=[itel, height, lat, lon], dims=['itel', 'height', 'lat', 'lon'])
A_H_Prof, A_ur_Prof, A_wr_Prof, A_P_Prof, A_lon_list= [], [], [], [], []
B_H_Prof, B_ur_Prof, B_wr_Prof, B_P_Prof, B_lon_list= [], [], [], [], []
extentlon = 50
for k in range(7) :
    print(k)
    ...
    A_lon_range = lon[(lon >= (A_lon - extentlon)) & (lon <= (A_lon + extentlon))]
    B_lon_range = lon[(lon >= (B_lon - extentlon)) & (lon <= (B_lon + extentlon))]
    Ai_H_prof = Hr.sel(itel = k, lon = A_lon_range, lat = A_lat)
    Bi_H_prof = Hr.sel(itel = k, lon = B_lon_range, lat = B_lat)
    Ai_P_prof = P.sel(itel = k, lon = A_lon_range, lat = A_lat)
    Bi_P_prof = P.sel(itel = k, lon = B_lon_range, lat = B_lat)
    Ai_ur_prof = ur.sel(itel = k, lon = A_lon_range, lat = A_lat)
    Bi_ur_prof = ur.sel(itel = k, lon = B_lon_range, lat = B_lat)
    Ai_wr_prof = wr.sel(itel = k, lon = A_lon_range, lat = A_lat)
    Bi_wr_prof = wr.sel(itel = k, lon = B_lon_range, lat = B_lat)
    A_H_Prof.append(Ai_H_prof)
    B_H_Prof.append(Bi_H_prof)
    A_P_Prof.append(Ai_P_prof)
    B_P_Prof.append(Bi_P_prof)
    A_ur_Prof.append(Ai_ur_prof)
    B_ur_Prof.append(Bi_ur_prof)
    A_wr_Prof.append(Ai_wr_prof)
    B_wr_Prof.append(Bi_wr_prof)
    A_lon_list.append(A_lon_range)
    B_lon_list.append(B_lon_range)


# ------- draw Picture ------ #
fig = plt.figure(figsize=(6, 14), dpi=400)
...
def draw_profile():
    ...
    for i in range(14):
        ...
        # 横坐标设置
        # axs[i].set_xlim(A_lon_list[i].min(), A_lon_list[i].max())
        # axs[i].set_xlabel('Latitude', fontsize=6)
        # w = axs[i].get_xticks()  # 获取横坐标值列表，主要为了调节大小
        # axs[i].set_xticklabels(w, fontdict={'fontsize': 6})
        if i%2 == 0:
            axs[i].set_xlim(A_lon_list[n].min(), A_lon_list[n].max())
            axs[i].set_xlabel('Longitude', fontsize=6)
            w = axs[i].get_xticks()  # 获取横坐标值列表，主要为了调节大小
            axs[i].set_xticklabels(w, fontdict={'fontsize': 6})

            drawH, drawv, draww, drawP= A_H_Prof[n], A_ur_Prof[n], A_wr_Prof[n], A_P_Prof[n]
            ax_cf1 = axs[i].contourf(A_lon_list[n],  height[0:10],  drawH[0:10, :], levels=np.arange(-0.025, 0.03, 0.005), zorder=0, extend = 'both', cmap='coolwarm')
            axs[i].contourf(A_lon_list[n],  height[0:10],  drawP[0:10, :], levels=[0, 0.05, 1], hatches=['...', None], zorder=1, colors="None")
            axs[i].quiver(A_lon_list[n],  height[0:10], drawv[0:10, :], draww[0:10, :])
            n = n+1
        else:
            axs[i].set_xlim(B_lon_list[m].min(), B_lon_list[m].max())
            axs[i].set_xlabel('Longitude', fontsize=6)
            w = axs[i].get_xticks()  # 获取横坐标值列表，主要为了调节大小
            axs[i].set_xticklabels(w, fontdict={'fontsize': 6})
            drawH, drawv, draww, drawP= B_H_Prof[m], B_ur_Prof[m], B_wr_Prof[m], B_P_Prof[m]
            ax_cf1 = axs[i].contourf(B_lon_list[m],  height[0:10],  drawH[0:10, :], levels=np.arange(-0.025, 0.03, 0.005), zorder=0, extend = 'both', cmap='coolwarm')
            axs[i].contourf(B_lon_list[m],  height[0:10],  drawP[0:10, :], levels=[0, 0.05, 1], hatches=['...', None], zorder=1, colors="None")
            axs[i].quiver(B_lon_list[m],  height[0:10], drawv[0:10, :], draww[0:10, :])
            m = m+1
    plt.show()

draw_profile()
fig.savefig(r'F:\6_Scientific_Research\3_Teleconnection\0 Start over again\1_Picture\3_Explanation\3_Profile\reg_Tel-W_Cir-W_UW-H_profile_Point.png')
