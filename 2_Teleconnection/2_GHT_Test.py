...
#--读取数据--#
# 相关系数以及P值筛选
f1 = xr.open_dataset(r'F:\7_Data\0_Science_Program\1_Graduation_Program\cor_p\r_all_lon_lat_1.nc' ,engine='netcdf4')
f2 = xr.open_dataset(r'F:\7_Data\0_Science_Program\1_Graduation_Program\cor_p\p_all_lon_lat.nc')
...
for ilat in range(27):
    for ilon in range(144):
        ...
        location = np.where(r_arr[ilat, ilon, :, :]==np.min(r_arr[ilat, ilon, :, :]))
        if len(location[0])>1:
...
def drawing(cyclic_lons, lat, cyclic_data):
    ...
    for i in range(len(Point_df.index)):
        ...
        ax.plot([A_lon, B_lon], [A_lat, B_lat], 'k', '.',transform=ccrs.PlateCarree(), linewidth=0.05)

    # 填充等值线和添加等值线、colobar
    ax_cf1 = ax.contourf(cyclic_lons, lat, cyclic_data, zorder=0, extend='both', transform=ccrs.PlateCarree(), \
                         cmap='hot', levels=np.arange(-0.7, -0.15, 0.05 ))  # 绘制等值线图
    # 添加colorbar
    ...
    # plt.savefig(r'F:\Pictures\1_Graduation_Project\3_Hot_Summer\Teleconnectivity matrix.png')
    plt.show()
