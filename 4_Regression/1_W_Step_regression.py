...
def drawing_model(cyclic_data1, cyclic_data2):
    ...
    # 图名
    ax1.set_title('Stepwise_linear_regression_RMSE')
    ax2.set_title('Stepwise_linear_regression_R2')
    # 刻度线形式
...
# 定义绘制预测SAT的hanshu
def drawing_SAT(cyclic_data1, cyclic_data2):
    # 设置字体为楷体,显示负号
    mpl.rcParams['font.sans-serif'] = ['KaiTi']
    mpl.rcParams['axes.unicode_minus'] = False  # 显示负号
    fig = plt.figure(figsize=(8, 6), dpi=400)
    ...
    ax2 = fig.add_subplot(2, 1, 2, projection=ccrs.PlateCarree(central_longitude=0))
    # 图名
    ax1.set_title('Stepwise_linear_SAT_2021_pred_W')
    ax2.set_title('Stepwise_linear_SAT_2022_pred_W')
    # 刻度线形式
    ax1.set_xticks([-180, -150, -120, -90, -60, -30,0, 30, 60, 90, 120, 150, 180]) #指定要显示的经纬度
    ax1.set_yticks([0, 30, 60, 90])
    ax1.xaxis.set_major_formatter(LongitudeFormatter()) #刻度格式转换为经纬度样式
    ...
    plt.show()
# drawing_SAT(SAT_pred_19, SAT_pred_20)

drawing_SAT(SAT_pred_21, SAT_pred_22)
