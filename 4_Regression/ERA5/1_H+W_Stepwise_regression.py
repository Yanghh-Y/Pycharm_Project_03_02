...
f1 = xr.open_dataset(r'F:\6_Scientific_Research\3_Teleconnection\1_Data\ERA5\ERA-Tem-GHT.nc')
...
SAT_pred_19,SAT_pred_20 = np.zeros((361, 1440)), np.zeros((361, 1440))
SAT_pred_21,SAT_pred_22 = np.zeros((361, 1440)), np.zeros((361, 1440))
# scores_r2_All,scores_rmse_All = np.zeros((37, 144)), np.zeros((37, 144))
for ilat in range(361):
    for ilon in range(1440):
        ...
        #                       usecols=['B1B2', 'C1C2', 'D1D2', 'E1E2', 'F1F2'])
        # TELI = pd.concat([TELIS_H, TELIS_W], axis=1)
        # col = ['W_A1A2', 'W_A1A3', 'W_B1B2', 'W_B1B3', 'W_C1C2', 'W_C1C3', 'W_E1E2', 'W_E1E3', 'B1B2', 'C1C2', 'D1D2', 'E1E2', 'F1F2']
        # TELI.columns = col
        TELIS = TELI
...
def drawing_SAT(cyclic_data1, cyclic_data2, name1, name2):
    ...
    # 图名
    ax1.set_title('ERA5-H+W_Stepwise_linear'+str(name1))
    ax2.set_title('ERA5-H+W_Stepwise_linear'+str(name2))
    # 刻度线形式
    ...
    plt.savefig(r'F:\6_Scientific_Research\3_Teleconnection\2_Picture\3_Pred\2_era5\H+W_Stepwise_linear_regression'+str(name1)+'_'+str(name2)+'.png')
