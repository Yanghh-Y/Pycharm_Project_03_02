...
for k in range(5): # 这一纬度是存放不同的遥相关指数对
    for i in range(37):
        for j in range(144):
            s[k, i, j], _, r[k, i, j], p[k ,i, j], _ = linregress(TELIS.iloc[:, 5+k], hgt_detrend[:, i, j])
...
def drawingcount(Data, P,abc):
    for nrow in range(5):
        # 图题
        axs2[nrow].set_title('reg(H-'+TEL_name[(nrow)]+',GHT)')
    ...
    fig.savefig(r'F:\6_Scientific_Research\3_Teleconnection\2_Picture\4_Explanation\reg_H-tel_GHT.png')
    plt.show()
