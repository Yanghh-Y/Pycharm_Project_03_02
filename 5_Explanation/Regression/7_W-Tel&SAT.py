...
# 计算回归
s,r,p = np.zeros((8, 91, 360)),np.zeros((8, 91, 360)),np.zeros((8, 91, 360))
for k in range(8): # 这一纬度是存放不同的遥相关指数对
    for i in range(91):
        for j in range(360):
            s[k, i, j], _, r[k, i, j], p[k ,i, j], _ = linregress(TELIS.iloc[:, 8+k], Tem_detrend[:, i, j])
...
ax1 = fig.add_subplot(8, 1, 1, projection=ccrs.PlateCarree(central_longitude=0))
ax2 = fig.add_subplot(8, 1, 2, projection=ccrs.PlateCarree(central_longitude=0))
ax3 = fig.add_subplot(8, 1, 3, projection=ccrs.PlateCarree(central_longitude=0))
ax4 = fig.add_subplot(8, 1, 4, projection=ccrs.PlateCarree(central_longitude=0))
ax5 = fig.add_subplot(8, 1, 5, projection=ccrs.PlateCarree(central_longitude=0))
ax6 = fig.add_subplot(8, 1, 6, projection=ccrs.PlateCarree(central_longitude=0))
ax7 = fig.add_subplot(8, 1, 7, projection=ccrs.PlateCarree(central_longitude=0))
ax8 = fig.add_subplot(8, 1, 8, projection=ccrs.PlateCarree(central_longitude=0))
...
def drawingcount(Data, P,abc):
    ...
    # fig.savefig(r'F:\6_Scientific_Research\3_Teleconnection\2_Picture\4_Explanation\reg_W-tel_Tem.png')
    plt.show()
