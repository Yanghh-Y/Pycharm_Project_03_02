...
u_year = f_u.uwnd.sel(level=p, lat=latrange).loc['1950-01-01':'2019-12-01'].mean(dim='time') * units('m/s')
v_year = f_v.vwnd.sel(level=p, lat=latrange).loc['1950-01-01':'2019-12-01'].mean(dim='time') * units('m/s')
# 位势高度转位势
...
ax.quiver(lon[::2], lat[::2], fx[::2,::2], fy[::2,::2], wind_climatic[::2,::2], transform=ccrs.PlateCarree(), scale=0.02, cmap=plt.cm.jet)
