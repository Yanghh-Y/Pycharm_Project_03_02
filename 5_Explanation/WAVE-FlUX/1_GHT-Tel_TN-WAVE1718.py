...
# z_reg_sum, u_reg_sum, v_reg_sum = np.zeros((37, 144)), np.zeros((37, 144)), np.zeros((37, 144))
# for i in range(37):
#        for j in range(144):
#               z_reg_sum[i, j], _, rz[i, j], pz[i, j], _ = linregress(teli.iloc[:, 6], z_sum[:, i, j]) # teli[:, 6]为BSI_std
#               u_reg_sum[i, j], _, ru[i, j], pu[i, j], _ = linregress(teli.iloc[:, 6], u_sum[:, i, j]) # teli[:, 6]为BSI_std
#               v_reg_sum[i, j], _, rv[i, j], pv[i, j], _ = linregress(teli.iloc[:, 6], v_sum[:, i, j]) # teli[:, 6]为BSI_std
# # 回归结果变成ndarray
# # hgt_reg 是去趋势过后的回归结果， z_reg是未去趋势过后的回归结果
# lon, lat = z_sum.lon, z_sum.lat
# hgt_reg = xr.DataArray(hgt_reg, coords=[lat, lon], dims=['lat', 'lon'])
# z_reg_sum = xr.DataArray(z_reg_sum, coords=[lat, lon], dims=['lat', 'lon'])
# u_reg_sum = xr.DataArray(u_reg_sum, coords=[lat, lon], dims=['lat', 'lon'])
# v_reg_sum = xr.DataArray(v_reg_sum, coords=[lat, lon], dims=['lat', 'lon'])
# hgt_reg = hgt_reg * units('m')
# z_reg_sum = z_reg_sum * units('m')
# u_reg_sum = u_reg_sum * units('m/s')
# v_reg_sum = v_reg_sum * units('m/s')
# 位势高度转位势
# 目标场z_sum -> Φ_sum  [37, 144]
Φ_sum = mpcalc.height_to_geopotential(z_sum) # [2, 37, 144]
...
# 背景场：1949-2018年summer z_year [70, 37, 144], u_year[37, 144], v_year[37, 144]
z_year = f_z.hgt.sel(level=p, lat=latrange).loc[(f_z.time.dt.month.isin([6, 7, 8]))].loc['1950-01-01':'2019-12-01'].groupby('time.year').mean(dim='time', skipna=True) * units('m')
u_year = f_u.uwnd.sel(level=p, lat=latrange).loc[(f_u.time.dt.month.isin([6, 7, 8]))].loc['1950-01-01':'2019-12-01'].mean(dim='time') * units('m/s')
v_year = f_v.vwnd.sel(level=p, lat=latrange).loc[(f_v.time.dt.month.isin([6, 7, 8]))].loc['1950-01-01':'2019-12-01'].mean(dim='time') * units('m/s')
# 位势高度转位势 Φ_year [70, 37, 144]
...
cosφ = np.cos(lat_rad)
# 位势的纬向偏差 Φ_sum [37, 144]  Φ_year [70, 37, 144]
Φ_prime = Φ_year.mean(dim='lon') + (-1 * Φ_sum)
...
# ax.quiver(lon[::2], lat[::2], fx[::2,::2]*100000, fy[::2,::2], wind_climatic[::2,::2], transform=ccrs.PlateCarree(), scale=0.1, cmap=plt.cm.jet)
