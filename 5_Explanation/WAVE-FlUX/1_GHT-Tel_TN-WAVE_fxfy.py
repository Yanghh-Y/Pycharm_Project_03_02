...
# latrange = f_z.lat
#---目标场：1949-2018年的夏季---#
z_sum = f_z.hgt.sel(level=p, lat=latrange).loc[(f_z.time.dt.month.isin([6, 7, 8]))].loc['1950-01-01':'2019-12-01'].groupby('time.year').mean(dim='time', skipna=True)
...
# 目标月和目标气压基本流场的气候态
wind_year = mpcalc.wind_speed(u_year, v_year) # [70, 37, 144]
cosφ = np.cos(lat_rad)
# 位势的纬向偏差 Φ_sum
...
# 准地转流函数相对于气候场的扰动
Ψ_prim = Φ_prime / f
year, lon, lat = Ψ_prim.year, Ψ_prim.lon, Ψ_prim.lat
# # 扰动流函数去线性趋势
...
# 给扰动准地转流函数的赋坐标和单位
Ψ_prime = xr.DataArray(Ψ_prim, coords=[year, lat, lon], dims=['year','lat', 'lon'])
...
# 去线性趋势 fx,fy [37, 40, 144]
fx_detrend = scipy.signal.detrend(fx[2:, :, :], axis=1, type='linear', overwrite_data=False) # fx_detrend [35, 70, 144]
fx_year_mean = np.nanmean(fx, axis=1) # fx_year_mean [37, 144]
for iyear in range(70):
    fx[2:, iyear, :] = fx_detrend[:, iyear, :] + fx_year_mean[2:, :]
fy_detrend = scipy.signal.detrend(fy[3:, :, :], axis=1, type='linear', overwrite_data=False)
fy_year_mean = np.nanmean(fy, axis=1) # fx_year_mean [37, 144]
for iyear in range(70):
    fy[3:, iyear, :] = fy_detrend[:, iyear, :] + fy_year_mean[3:, :]
# fx,fy 与 BSI_STD 回归
pfx,rfx,fxreg = np.zeros((37, 144)),np.zeros((37, 144)),np.zeros((37, 144))
pfy,rfy,fyreg = np.zeros((37, 144)),np.zeros((37, 144)),np.zeros((37, 144))
for i in range(37):
       for j in range(144):
              fxreg[i, j], _, rfx[i, j], pfx[i, j], _ = linregress(teli.iloc[:, 6], fx[i, :, j])
              fyreg[i, j], _, rfy[i, j], pfy[i, j], _ = linregress(teli.iloc[:, 6], fy[i, :, j])
...
ax.quiver(lon[::3], lat[::3], fxreg[::3,::3], fyreg[::3,::3]/10000, transform=ccrs.PlateCarree(), scale=70)
# ax.quiver(lon[::2], lat[::2], fxreg[::2,::2], fyreg[::2,::2], wind_climatic[::2,::2], transform=ccrs.PlateCarree(), scale=2, cmap=plt.cm.jet)
