...
f2 = xr.open_dataset(r'F:\6_Scientific_Research\3_Teleconnection\1_Data\R_P\Cir_W_De_Lattice_p_all_lon_lat.nc')
r = f1.r.sel(ilat=np.arange(90, 22.5, -2.5), dlat=np.arange(90, 22.5, -2.5))
p = f2.p.sel(ilat=np.arange(90, 22.5, -2.5), dlat=np.arange(90, 22.5, -2.5))
r_arr = np.array(r)
p_arr = np.array(p)
...
ds.to_netcdf(r'F:\6_Scientific_Research\3_Teleconnection\1_Data\R_P\cir_W_De_Lattice_r_min_arr.nc')
...
Point_df.to_csv(r'F:\6_Scientific_Research\3_Teleconnection\1_Data\R_P\Cir_W_De_Lattice_Point.csv', )
...
def drawing(cyclic_lons, lat, cyclic_data):
    ...
    for i in range(len(Point_df.index)):
        ...
        # ax.plot([A_lon, B_lon], [A_lat, B_lat], 'b', '.', transform=ccrs.PlateCarree(), linewidth=0.1)

    # 设置图名和边框
    picture_title = "Teleconnectivity matrix"
    ax.set_title(picture_title, fontsize=15, pad=25)  # 图名title
    # 填充等值线和添加等值线、colobar
    ax_cf1 = ax.contourf(cyclic_lons, lat, cyclic_data, zorder=0, extend='both', transform=ccrs.PlateCarree(), \
                         cmap='hot', levels=np.arange(-0.6, 0.2, 0.025 ))  # 绘制等值线图
    # 添加colorbar
    ...
    # plt.savefig(r'F:\6_Scientific_Research\3_Teleconnection\2_Picture\2_Teleconnection\Cir_W_De_Lettice_Teleconnectivity matrix.png')
    plt.show()
